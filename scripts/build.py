...
from html_templates import (
    render_page, breadcrumb, jsonld_article, svg_logo, relative_root
)
...
def badge(status: str) -> str:
    ...
    return f'<span class="badge badge--{css}">{status or "unknown"}</span>'
...
def build_homepage(data: dict, config: dict, dist: Path, env: str) -> None:
    ...
    standards_html = "".join(
        f'<a class="standards-bar__item" href="{s["url"]}" target="_blank" rel="noopener" title="{s["description"]}">{s["name"]}</a>'
    ...
    for p in data["pillars"]:
        ...
        href = f"pillars/{pid}/index.html"
        pillar_cards += f"""
        <a class="pillar-card" href="{href}">
          <div class="pillar-card__id">{pid}</div>
          <div class="pillar-card__title">{p["name"]}</div>
          <div class="pillar-card__desc">{p.get("description", "")}</div>
...
<section class="hero">
  ...
  <h1 class="hero__title">{config["site"]["name"]}</h1>
  <p class="hero__tagline">{config["site"]["tagline"]}</p>
  <p class="hero__description">{config["site"]["description"]}</p>
  <div class="standards-bar" aria-label="Aligned standards">
    {standards_html}
  </div>
  <a class="btn btn--primary hero__cta" href="{policy_repo}" target="_blank" rel="noopener">Contribute on GitHub</a>
</section>
...
def build_pillar_page(pillar: dict, config: dict, dist: Path) -> None:
    ...
    canonical = f"{base_url}/pillars/{pid}/index.html"
    ...
    for sub in pillar["sub_pillars"]:
        sid = sub["id"]
        ref_badges = "".join(
            f'<span class="tag">{ref}</span>'
            for ref in (sub.get("policy_refs") or [])
        )
        sub_cards += f"""
        <a class="pillar-card" href="{sid}/index.html">
          <div class="pillar-card__id">{sid}</div>
          <div class="pillar-card__title">{sub["name"]}</div>
          <div class="pillar-card__desc">{sub.get("description", "")}</div>
          <div class="pillar-card__footer">
            <div>{ref_badges}</div>
    ...
    for label, key in [("Owner", "owner"), ("Effective", "effective_date"), ("Review", "review_date"), ("Version", "version")]:
        if pillar.get(key):
            meta_html += f'<div class="policy-meta__item"><span>{label}</span><span class="policy-meta__value">{pillar[key]}</span></div>'

    body = f"""
<article>
  <header class="policy-header">
    <div class="policy-header__id">{pid}</div>
    <h1 class="policy-header__name">{pillar["name"]}</h1>
    <div class="policy-meta">{meta_html}{badge(pillar.get("status","active"))}</div>
    <p style="color:var(--color-text-muted);margin-top:0.75rem;line-height:1.7">{pillar.get("description","")}</p>
  </header>
...
def build_sub_pillar_page(sub: dict, pillar: dict, config: dict, dist: Path) -> None:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    canonical = f"{base_url}/pillars/{pid}/{sid}/index.html"
    root = relative_root(canonical, base_url)
    ...
    for label, key in [("Owner", "owner"), ("Effective", "effective_date"), ("Review", "review_date"), ("Version", "version")]:
        if sub.get(key):
            meta_html += f'<div class="policy-meta__item"><span>{label}</span><span class="policy-meta__value">{sub[key]}</span></div>'

    tags_html = "".join(f'<span class="tag">{t}</span>' for t in (sub.get("tags") or []))
    ...
    for pol in (sub.get("resolved_policies") or []):
        ...
        for label, key in [("Owner", "owner"), ("Effective", "effective_date"), ("Version", "version")]:
            if pol.get(key):
                pol_meta += f'<div class="policy-meta__item"><span>{label}</span><span class="policy-meta__value">{pol[key]}</span></div>'
        refs_html += f"""
<div class="policy-card" data-policy-id="{pol["id"]}">
  <div class="policy-card__header">
    ...
    <div style="display:flex;gap:0.75rem;align-items:center">
      ...
      <span style="font-size:0.8rem;color:var(--color-text-muted)">v{pol.get("version","")}</span>
...
<article data-page="sub-pillar">
  <header class="policy-header">
    <div class="policy-header__id">{pillar["name"]} / {sid}</div>
    <h1 class="policy-header__name">{sub["name"]}</h1>
    ...
    <div class="policy-actions">
      <a class="btn btn--ghost" href="{edit_url}" target="_blank" rel="noopener">✏ Edit on GitHub</a>
      <a class="btn btn--ghost" href="{view_url}" target="_blank" rel="noopener">⎆ View on GitHub</a>
    </div>
  </header>
  ...
  <div class="version-history" id="version-history">
    <button class="version-history__toggle" data-policy-ids="{",".join(p["id"] for p in sub.get("resolved_policies",[]))}" onclick="toggleVersionHistory(this)">
      📋 Version History
    </button>
...
def build_sitemap(data: dict, config: dict, dist: Path) -> None:
    ...
    for pillar in data["pillars"]:
        pid = pillar["id"]
        urls.append(f"{base_url}/pillars/{pid}/index.html")
        for sub in pillar["sub_pillars"]:
            urls.append(f"{base_url}/pillars/{pid}/{sub['id']}/index.html")
